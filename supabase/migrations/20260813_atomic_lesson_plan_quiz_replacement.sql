-- Replace a generated lesson-plan quiz set in one PostgreSQL transaction.
-- Generation and semantic validation remain outside this RPC; the browser calls
-- it only after every subject has a complete validated 3-by-4 replacement.
...
BEGIN
  ...
  IF v_plan_teacher_id IS DISTINCT FROM v_profile_id THEN
  ...
  IF EXISTS (
    ...
    WHERE quiz.value ->> 'lesson_plan_id' IS DISTINCT FROM p_plan_id
       OR quiz.value ->> 'teacherId' IS DISTINCT FROM v_profile_id
    ...
    WHERE question.value ->> 'source_lesson_plan_id' IS DISTINCT FROM p_plan_id
       OR question.value ->> 'teacherId' IS DISTINCT FROM v_profile_id
