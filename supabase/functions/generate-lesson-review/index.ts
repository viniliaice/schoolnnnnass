...
/**
 * Emit bounded lifecycle metadata only. Never include IDs, prompts, lesson
 * payloads, generated content, provider response bodies, keys, or reasoning.
 */
function logLifecycle(
  phase: 'received' | 'background-started' | 'provider-started' | 'provider-finished' | 'validation' | 'persistence-started' | 'persistence-succeeded' | 'stale-discarded' | 'failure',
  metadata: Record<string, string | number | boolean | null> = {},
): void {
  console.log('[generate-lesson-review] lifecycle', { phase, ...metadata });
}
...
export async function callLLM(
  ...
  logLifecycle('provider-started', requestMetadata);
  console.log('[generate-lesson-review] provider request', requestMetadata);
  ...
  try {
    ...
    logLifecycle('provider-finished', responseMetadata);
    console.log('[generate-lesson-review] provider response metadata', responseMetadata);
    if (!response.ok) {
      const error: ProviderAttemptError = response.status === 429
        ? new RateLimitError('Rate limited')
        : response.status === 401
          ? new APIKeyError('Invalid API key')
          : new Error(`Provider API error: HTTP ${response.status}`);
    ...
    try {
      ...
      logLifecycle('validation', {
        ...
        periodReviewCount: shape.periodReviewCount,
        responseChars,
        responseBytes,
        latencyMs: providerError.latencyMs ?? null,
      });
    ...
    console.log('[generate-lesson-review] provider attempt succeeded', {
      provider: route.provider,
      model: route.model,
      strategy,
      attempt,
    ...
    logLifecycle('failure', {
      scope: 'provider-attempt',
      provider: route.provider,
      model: route.model,
...
export async function callLLMWithRetry(
  ...
  for (let attempt = 0; attempt <= PROVIDER_MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      if (!retriableHttpStatus || attempt === PROVIDER_MAX_RETRIES) break;
...
async function markPlanFailed(
  ...
  const { data: persisted, error: persistError } = await supabase.rpc(
    'mark_lesson_plan_ai_review_attempt_failed',
    {
      p_plan_id: planId,
  ...
  if (persistError) {
    console.error('[generate-lesson-review] failed to persist attempt failure', {
      errorCode: 'FAILURE_PERSISTENCE_ERROR',
    });
    return;
  }
  if (!persisted) {
    logLifecycle('stale-discarded', {
      stage: 'failure-persistence',
...
async function generateAndPersistReview(job: ReviewJob): Promise<void> {
  ...
  logLifecycle('background-started', {
    periodCount: payload.periods.length,
    unitContextCount: payload.unit_contexts.length,
  ...
  try {
    ...
    let providerUsed: ProviderRoute['provider'] = 'gemini';
    ...
    logLifecycle('persistence-started', {
      provider: providerUsed,
      model: modelUsed,
      periodReviewCount: periodRows.length,
      executionMs: Date.now() - diagnostics.startedAt,
    });
    const { data: persisted, error: persistError } = await supabase.rpc(
    ...
    logLifecycle('persistence-succeeded', {
      provider: providerUsed,
      model: modelUsed,
      periodReviewCount: periodRows.length,
      executionMs: Date.now() - diagnostics.startedAt,
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const supabaseServiceKey = runtimeEnv.get('SUPABASE_SERVICE_ROLE_KEY')!;
    ...
    const geminiApiKey = runtimeEnv.get('GEMINI_API_KEY');
    if (!geminiApiKey) {
      const error = new APIKeyError('The Gemini API key for AI review is not configured.');
      logLifecycle('failure', {
