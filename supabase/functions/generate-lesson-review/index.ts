...
const MAX_PROVIDER_ATTEMPTS_PER_REQUEST = 6;
...
export async function callLLM(
  ...
  console.log('[generate-lesson-review] provider started', {
  ...
  const headers: Record<string, string> = route.provider === 'openrouter'
    ? {
  ...
  const requestBody = route.provider === 'openrouter'
    ? {
      model: route.model,
      messages: [
  ...
  try {
    ...
    if (!response.ok) {
      const error: ProviderAttemptError = response.status === 429
        ...
        : response.status === 401
          ...
          : new Error(`${route.provider === 'openrouter' ? 'OpenRouter' : 'Gemini'} API error: HTTP ${response.status}`);
    ...
    console.log('[generate-lesson-review] provider finished', {
      provider: route.provider,
      model: route.model,
      strategy,
      attempt,
    ...
    console.error('[generate-lesson-review] provider finished', {
      provider: route.provider,
      model: route.model,
      strategy,
      attempt: providerError.attempt ?? attempt,
...
async function markPlanFailed(
  ...
  if (markError) {
    console.error('[generate-lesson-review] failure persistence failed', {
      errorCode: 'FAILURE_RPC_ERROR',
    });
    return;
  }
  if (!marked) {
    console.info('[generate-lesson-review] stale failure discarded');
  }
}
...
async function generateAndPersistReview(job: ReviewJob): Promise<void> {
  ...
  console.log('[generate-lesson-review] background started', {
  ...
  try {
    ...
    let retryCount = 0;
    let providerUsed: ProviderRoute['provider'] = 'openrouter';
    let modelUsed = NEMOTRON_MODEL;
    ...
    console.log('[generate-lesson-review] persistence started', {
    ...
    console.log('[generate-lesson-review] persistence succeeded', {
      provider: providerUsed,
      model: modelUsed,
      periodReviewCount: periodRows.length,
      executionMs: Date.now() - diagnostics.startedAt,
...
Deno.serve(async (req: Request) => {
  ...
  console.log('[generate-lesson-review] request received', {
    method: req.method,
  ...
  try {
    ...
    const openRouterApiKey = runtimeEnv.get('OPENROUTER_API_KEY');
    const geminiApiKey = runtimeEnv.get('GEMINI_API_KEY');
    if (!openRouterApiKey && !geminiApiKey) {
      const error = new APIKeyError('No lesson-review provider key is configured.');
      console.error('[generate-lesson-review] provider configuration failed', {
        errorCode: 'MISSING_PROVIDER_KEY',
